	...
	for child in state.get_children():
		if child is StateChartState:
			...
			new_tree_item.set_text(0, start_str + child.name + " (" + str(child.get_script().get_global_name()) + ")")
				...
				#new_transition_tree_item.set_text(0, "➡️ " + child.current_transition.name + " -> " + child.current_transition.to.name + " [" + str(child.current_transition_timer.time_left).pad_decimals(2) + "]")
